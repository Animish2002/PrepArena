...
const TYPE_STYLES: Record<Toast['type'], string> = {
  success: 'bg-emerald-50 dark:bg-emerald-950/60 border-emerald-500/30 text-emerald-800 dark:text-emerald-300',
  info: 'bg-blue-50 dark:bg-blue-950/60 border-blue-500/30 text-blue-800 dark:text-blue-300',
  battle: 'bg-violet-50 dark:bg-violet-950/60 border-violet-500/30 text-violet-800 dark:text-violet-300',
}
...
function ToastItem({ toast }: { toast: Toast }) {
  ...
  return (
    <motion.div
      ...
      className={`flex items-start gap-3 px-4 py-3 rounded-xl border shadow-lg text-sm font-medium max-w-xs w-full pointer-events-auto ${TYPE_STYLES[toast.type]}`}
    >
