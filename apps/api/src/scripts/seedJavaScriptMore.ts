...
function t(
  ...
  return {
    ...
    tags: JSON.stringify([topic.toLowerCase().replace(/ /g, '-'), 'javascript']),
    questionType: 'theory', subject: 'javascript',
...
function mcq(
  ...
  return {
    ...
    tags: JSON.stringify([topic.toLowerCase().replace(/ /g, '-'), 'javascript', 'mcq']),
    questionType: 'mcq', subject: 'javascript',
    content: JSON.stringify({ question, options, correct_index, explanation }),
    contentSource: 'internal',
  }
}

// ─── CLOSURES & SCOPE (20 questions) ──────────────────────────────────────────

const CLOSURES: Row[] = [

t('js-ext-t-001', 'What is a closure and how does it work?', 'Closures', 'medium', `
...
t('js-ext-t-002', 'Module pattern and IIFE — encapsulation with closures', 'Closures', 'medium', `
## Module Pattern & IIFE
...
t('js-ext-t-003', 'Currying and partial application', 'Closures', 'hard', `
...
mcq('js-ext-m-001', 'Closure over loop variable', 'Closures', 'medium',
  'What does the following code output?\n\n```js\nconst funcs = [];\nfor (var i = 0; i < 3; i++) {\n  funcs.push(() => i);\n}\nconsole.log(funcs[0](), funcs[1](), funcs[2]());\n```',
  ['0 1 2', '3 3 3', '0 0 0', 'undefined undefined undefined'], 1,
  'var is function-scoped, so all three closures share the same `i`. After the loop, i is 3. Use `let` for block-scoped bindings.'),

mcq('js-ext-m-002', 'Closure identity', 'Closures', 'easy',
  'Which statement about closures is FALSE?',
  [
    'A closure can access variables in its outer function after the outer function returns',
    'Each closure gets its own copy of the outer variable',
    'Closures are created every time a function is created in JavaScript',
    'Closures keep referenced variables alive in memory',
  ], 1,
  'Closures capture a reference to the outer scope\'s variable, not a copy. Multiple closures from the same outer scope share the same variable.'),

t('js-ext-t-004', 'Memoization using closures', 'Closures', 'medium', `
## Memoization
...
// ─── ASYNC & PROMISES (25 questions) ─────────────────────────────────────────

const ASYNC: Row[] = [

t('js-ext-t-005', 'Promise states, chaining & error propagation', 'Async & Event Loop', 'medium', `
## Promises
...
t('js-ext-t-006', 'Promise combinators — all, allSettled, race, any', 'Async & Event Loop', 'medium', `
## Promise Combinators
...
t('js-ext-t-007', 'async/await — execution model and error handling', 'Async & Event Loop', 'medium', `
## async/await
...
t('js-ext-t-008', 'The Event Loop in depth — microtasks vs macrotasks', 'Async & Event Loop', 'hard', `
...
t('js-ext-t-009', 'Generators and async iteration', 'Async & Event Loop', 'hard', `
## Generators
...
mcq('js-ext-m-003', 'Promise.all behaviour', 'Async & Event Loop', 'medium',
  'What happens when one promise in Promise.all rejects?',
  [
    'It waits for all promises to settle, then throws',
    'It immediately rejects with the first rejection reason',
    'It ignores the rejection and resolves with the other values',
    'It retries the rejected promise once',
  ], 1,
  'Promise.all is fail-fast — as soon as any promise rejects, the returned promise rejects immediately. Use Promise.allSettled if you want to wait for all.'),

mcq('js-ext-m-004', 'async function return value', 'Async & Event Loop', 'easy',
...
mcq('js-ext-m-005', 'Microtask vs macrotask order', 'Async & Event Loop', 'hard',
  'What is the output of:\n```js\nsetTimeout(() => console.log("A"), 0);\nPromise.resolve().then(() => console.log("B"));\nconsole.log("C");\n```',
  ['C A B', 'C B A', 'A B C', 'B C A'], 1,
  'Sync runs first (C), then microtasks (Promise .then → B), then macrotasks (setTimeout → A).'),

]

// ─── FUNCTIONS & this (20 questions) ──────────────────────────────────────────

const FUNCTIONS: Row[] = [

t('js-ext-t-010', 'call, apply, bind — explicit this binding', 'Functions', 'medium', `
## call / apply / bind
...
t('js-ext-t-011', 'Arrow functions vs regular functions — 6 key differences', 'Functions', 'medium', `
...
t('js-ext-t-012', 'Function composition and pipe', 'Functions', 'hard', `
...
mcq('js-ext-m-006', 'this in arrow function', 'Functions', 'medium',
  'What does the following log?\n```js\nconst obj = {\n  val: 42,\n  getVal: () => this.val,\n};\nconsole.log(obj.getVal());\n```',
  ['42', 'undefined', 'TypeError', 'null'], 1,
  'Arrow functions capture `this` from the enclosing lexical scope. At the top level (or in a module), `this` is `undefined` (strict mode) or `window`. It is NOT the object literal.'),

mcq('js-ext-m-007', 'bind return value', 'Functions', 'easy',
  'What does Function.prototype.bind() return?',
  ['The result of calling the function', 'A new function with bound this', 'undefined', 'A Promise'],
  1,
  'bind() returns a NEW function with `this` permanently bound. It does not invoke the function immediately.'),

t('js-ext-t-013', 'Debounce and throttle — controlling function call rate', 'Functions', 'medium', `
## Debounce & Throttle
...
// ─── PROTOTYPES & CLASSES (20 questions) ──────────────────────────────────────

const PROTOTYPES: Row[] = [

t('js-ext-t-014', 'Prototype chain — how property lookup works', 'Prototypes & Classes', 'medium', `
## Prototype Chain
...
t('js-ext-t-015', 'ES6 Classes — syntax, inheritance, static, private', 'Prototypes & Classes', 'medium', `
## ES6 Classes
...
t('js-ext-t-016', 'Object.create, Object.assign, spread — object manipulation', 'Prototypes & Classes', 'easy', `
## Object Manipulation
...
mcq('js-ext-m-008', 'instanceof check', 'Prototypes & Classes', 'medium',
  'What does `instanceof` check?',
  [
    'Whether the object was created with a specific constructor',
    'Whether the constructor\'s prototype is in the object\'s prototype chain',
    'Whether the object has all the same properties',
    'The typeof the object',
  ], 1,
  '`instanceof` walks the prototype chain checking if `Constructor.prototype` appears anywhere in it. It does NOT check the constructor itself.'),

mcq('js-ext-m-009', 'Class vs function hoisting', 'Prototypes & Classes', 'medium',
  'What happens if you use a class before it is declared?',
  [
    'It works fine — classes are hoisted like functions',
    'ReferenceError — class declarations are in the TDZ',
    'undefined is returned',
    'A SyntaxError at parse time',
  ], 1,
  'Class declarations are hoisted but NOT initialized — they are in the Temporal Dead Zone (TDZ), just like `let` and `const`.'),
...
t('js-ext-t-017', 'Destructuring — arrays, objects, defaults, rename, nested', 'ES6+ Features', 'easy', `
## Destructuring
...
t('js-ext-t-018', 'Symbols, Iterators & the Iteration Protocol', 'ES6+ Features', 'hard', `
...
t('js-ext-t-019', 'WeakMap and WeakSet — use cases and garbage collection', 'ES6+ Features', 'hard', `
## WeakMap & WeakSet
...
t('js-ext-t-020', 'Proxy and Reflect — meta-programming', 'ES6+ Features', 'hard', `
## Proxy & Reflect
...
mcq('js-ext-m-010', 'Optional chaining', 'ES6+ Features', 'easy',
  'What does `user?.address?.city` return if `user` is `null`?',
  ['null', 'undefined', 'TypeError', '""'], 1,
  'Optional chaining (?.) short-circuits and returns `undefined` (not null) if any part of the chain is null or undefined.'),

mcq('js-ext-m-011', 'Nullish coalescing', 'ES6+ Features', 'easy',
  'What does `0 ?? "default"` return?',
  ['"default"', '0', 'null', 'undefined'], 1,
  'Nullish coalescing (??) only falls back for null/undefined — not for other falsy values like 0, "", or false. Contrast with ||: `0 || "default"` returns "default".'),

mcq('js-ext-m-012', 'Spread vs rest', 'ES6+ Features', 'easy',
  'In `function f(a, ...b) {}`, what is `b` in `f(1, 2, 3)`?',
  ['[2, 3]', '2', '[1, 2, 3]', 'undefined'], 0,
  'The rest parameter (...b) collects remaining arguments into an array. a=1, b=[2,3].'),

]

// ─── ARRAYS & COLLECTIONS (20 questions) ─────────────────────────────────────

const ARRAYS: Row[] = [

t('js-ext-t-021', 'Array methods mastery — map, filter, reduce, find, flat', 'Arrays & Collections', 'easy', `
## Array Methods
...
t('js-ext-t-022', 'Map and Set — when and why over plain objects/arrays', 'Arrays & Collections', 'medium', `
## Map & Set
...
mcq('js-ext-m-013', 'Array sort stability', 'Arrays & Collections', 'medium',
  'What is the default sort behavior of Array.prototype.sort()?',
  [
    'Sorts numerically ascending',
    'Sorts by Unicode code point as strings, mutates in place',
    'Returns a new sorted array without mutation',
    'Sorts by length',
  ], 1,
  'sort() is in-place (mutates) and by default converts elements to strings and compares Unicode code points. `[10, 2, 1].sort()` gives `[1, 10, 2]`. Always provide a comparator for numbers.'),

mcq('js-ext-m-014', 'Array vs Set lookup', 'Arrays & Collections', 'easy',
  'What is the time complexity of `Set.prototype.has()` vs `Array.prototype.includes()`?',
  ['Both O(1)', 'Set O(1), Array O(n)', 'Both O(n)', 'Set O(log n), Array O(n)'],
  1,
  'Set uses a hash table internally, giving O(1) average lookup. Array.includes() does a linear scan — O(n).'),

]

// ─── ERROR HANDLING & MODULES (15 questions) ──────────────────────────────────

const ERRORS: Row[] = [

t('js-ext-t-023', 'Error types and custom errors', 'Error Handling', 'medium', `
...
t('js-ext-t-024', 'ESM vs CommonJS — modules in depth', 'Modules', 'medium', `
## ESM vs CommonJS
...
mcq('js-ext-m-015', 'import hoisting', 'Modules', 'medium',
  'Where can `import` statements appear in an ES module?',
  [
    'Anywhere in the file',
    'Only at the top level (not inside if/for/functions)',
    'Inside functions only',
    'Inside class bodies',
  ], 1,
  'Static `import` statements must be at the top level of a module — they are hoisted and evaluated before any code runs. Use dynamic `import()` for conditional/lazy loading.'),
...
t('js-ext-t-025', 'TypeScript — type vs interface, when to use each', 'TypeScript', 'medium', `
...
t('js-ext-t-026', 'TypeScript generics — constraints, defaults, conditional types', 'TypeScript', 'hard', `
...
mcq('js-ext-m-016', 'TypeScript unknown vs any', 'TypeScript', 'medium',
  'What is the key difference between `any` and `unknown` in TypeScript?',
  [
    'any is for runtime errors, unknown is for compile errors',
    'unknown requires type narrowing before use; any bypasses all type checks',
    'unknown can only hold primitive values',
    'any is deprecated in TypeScript 5+',
  ], 1,
  '`unknown` is the type-safe counterpart of `any`. You cannot call methods or access properties on `unknown` without first narrowing the type (typeof, instanceof, type guard). `any` disables type checking entirely.'),

mcq('js-ext-m-017', 'TypeScript Partial utility type', 'TypeScript', 'easy',
  'What does `Partial<User>` produce if `User = { id: number; name: string }`?',
  [
    '{ id?: number; name?: string }',
    '{ id: number | null; name: string | null }',
    'never',
    '{ id: number; name: string } & { id?: never; name?: never }',
  ], 0,
  'Partial<T> maps every property of T to optional (adds `?`). It is a built-in mapped type.'),

]

// ─── Export all ───────────────────────────────────────────────────────────────

export const allJavaScriptMoreProblems: Row[] = [
  ...CLOSURES,
  ...ASYNC,
  ...FUNCTIONS,
  ...PROTOTYPES,
