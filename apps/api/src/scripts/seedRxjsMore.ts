...
function t(id: string, title: string, topic: string, difficulty: 'easy'|'medium'|'hard', content: string, subtopic?: string): Row {
  return {
    ...
    tags: JSON.stringify([topic.toLowerCase().replace(/ /g, '-'), 'rxjs']),
...
function mcq(id: string, title: string, topic: string, difficulty: 'easy'|'medium'|'hard', question: string, options: string[], correct_index: number, explanation: string): Row {
  return {
    ...
    tags: JSON.stringify([topic.toLowerCase().replace(/ /g, '-'), 'rxjs', 'mcq']),
...
// ─── OBSERVABLES & CORE CONCEPTS (20 questions) ───────────────────────────────

const OBSERVABLES: Row[] = [

t('rxjs-ext-t-001', 'Observable vs Promise — key differences', 'Observables', 'medium', `
...
t('rxjs-ext-t-002', 'flattening operators — mergeMap, switchMap, concatMap, exhaustMap', 'Observables', 'hard', `
...
t('rxjs-ext-t-003', 'Pipeable operators — common patterns', 'Observables', 'medium', `
...
mcq('rxjs-ext-m-001', 'switchMap vs mergeMap', 'Observables', 'hard',
  'A user types in a search box. Each keystroke triggers an API call. Which operator should you use to ensure only the latest query\'s result is used?',
  ['mergeMap', 'concatMap', 'switchMap', 'exhaustMap'], 2,
  'switchMap cancels the previous inner Observable when a new value arrives. This prevents stale results from earlier queries from overwriting newer ones.'),

mcq('rxjs-ext-m-002', 'Observable laziness', 'Observables', 'easy',
  'When does a cold Observable start executing?',
  [
    'When it is created with new Observable()',
    'When subscribe() is called',
    'When the first operator is added via pipe()',
    'On the next event loop tick',
  ], 1,
  'Observables are lazy — they do nothing until subscribe() is called. Each subscription starts a fresh, independent execution.'),
...
t('rxjs-ext-t-004', 'Subject types — Subject, BehaviorSubject, ReplaySubject, AsyncSubject', 'Subjects', 'medium', `
## RxJS Subject Types
...
t('rxjs-ext-t-005', 'State management with BehaviorSubject — Angular service store', 'Subjects', 'medium', `
...
mcq('rxjs-ext-m-003', 'BehaviorSubject initial value', 'Subjects', 'easy',
  'What does a new subscriber to a BehaviorSubject receive immediately?',
  [
    'Nothing — it waits for the next .next() call',
    'The current value (or the initial value)',
    'All values that were ever emitted',
    'The last two values',
  ], 1,
  'BehaviorSubject always has a current value and emits it synchronously to every new subscriber. This is its key difference from Subject.'),

]

// ─── ERROR HANDLING & RETRY (20 questions) ────────────────────────────────────

const ERRORS: Row[] = [

t('rxjs-ext-t-006', 'catchError, retry, retryWhen — resilient streams', 'Error Handling', 'hard', `
...
t('rxjs-ext-t-007', 'takeUntil pattern — managing subscriptions and preventing memory leaks', 'Error Handling', 'medium', `
...
mcq('rxjs-ext-m-004', 'catchError return type', 'Error Handling', 'medium',
  'What must catchError\'s callback return?',
  [
    'The original value that caused the error',
    'An Observable (a new stream to continue with)',
    'A Promise',
    'null or undefined to swallow the error',
  ], 1,
  'catchError receives the error and must return an Observable. Returning of(fallback) recovers, EMPTY completes, throwError() propagates. You cannot return a plain value.'),

]

// ─── COMBINING OBSERVABLES (20 questions) ─────────────────────────────────────

const COMBINING: Row[] = [

t('rxjs-ext-t-008', 'Combining observables — combineLatest, forkJoin, zip, withLatestFrom', 'Combining Streams', 'hard', `
...
t('rxjs-ext-t-009', 'startWith, pairwise, scan — stateful operators', 'Combining Streams', 'medium', `
...
mcq('rxjs-ext-m-005', 'combineLatest behaviour', 'Combining Streams', 'medium',
  'combineLatest([a$, b$]) — when does it emit its first value?',
  [
    'As soon as either a$ or b$ emits',
    'Only after both a$ and b$ have each emitted at least one value',
    'When both a$ and b$ complete',
    'On every emission from a$ only',
  ], 1,
  'combineLatest waits until ALL source Observables have emitted at least one value before it emits. After that, it emits whenever ANY source emits.'),

]

// ─── REAL-WORLD PATTERNS (15 questions) ───────────────────────────────────────

const PATTERNS: Row[] = [

t('rxjs-ext-t-010', 'shareReplay — multicasting and caching HTTP requests', 'Real-world Patterns', 'medium', `
## shareReplay
...
t('rxjs-ext-t-011', 'RxJS in Angular — unsubscribe strategies comparison', 'Real-world Patterns', 'medium', `
...
mcq('rxjs-ext-m-006', 'shareReplay purpose', 'Real-world Patterns', 'medium',
  'Why would you use shareReplay(1) on an HTTP request observable?',
  [
    'To make the request retry once on failure',
    'To share a single HTTP execution among multiple subscribers and cache the result',
    'To delay the HTTP request by 1 second',
    'To make the observable hot',
  ], 1,
  'shareReplay(1) multicasts the execution (so N subscribers trigger only 1 HTTP request) and caches the latest result so late subscribers get it immediately without a new request.'),

]

// ─── Export ───────────────────────────────────────────────────────────────────

export const allRxjsMoreProblems: Row[] = [
  ...OBSERVABLES,
  ...SUBJECTS,
  ...ERRORS,
  ...COMBINING,
