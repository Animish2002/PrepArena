...
function t(id: string, title: string, topic: string, difficulty: 'easy'|'medium'|'hard', content: string, subtopic?: string): Row {
  return {
    ...
    tags: JSON.stringify([topic.toLowerCase().replace(/ /g, '-'), 'angular']),
...
function mcq(id: string, title: string, topic: string, difficulty: 'easy'|'medium'|'hard', question: string, options: string[], correct_index: number, explanation: string): Row {
  return {
    ...
    tags: JSON.stringify([topic.toLowerCase().replace(/ /g, '-'), 'angular', 'mcq']),
...
// ─── COMPONENTS & LIFECYCLE (20 questions) ────────────────────────────────────

const COMPONENTS: Row[] = [

t('angular-ext-t-001', 'Angular component lifecycle hooks — complete guide', 'Components & Lifecycle', 'medium', `
...
t('angular-ext-t-002', '@Input and @Output — component communication', 'Components & Lifecycle', 'easy', `
## @Input & @Output
...
t('angular-ext-t-003', 'Content projection — ng-content and named slots', 'Components & Lifecycle', 'medium', `
## Content Projection
...
mcq('angular-ext-m-001', 'ngOnChanges vs ngOnInit', 'Components & Lifecycle', 'medium',
  ...
  'ngOnChanges fires before ngOnInit AND every time a bound @Input property changes. It receives a SimpleChanges object with previous and current values.'),

mcq('angular-ext-m-002', 'ViewChild availability', 'Components & Lifecycle', 'medium',
  'In which lifecycle hook is @ViewChild first available?',
  ['ngOnInit', 'ngAfterContentInit', 'ngAfterViewInit', 'ngDoCheck'],
  2,
  '@ViewChild references are populated after the view is initialized, which happens in ngAfterViewInit. Accessing them in ngOnInit will return undefined.'),

]

// ─── SERVICES & DEPENDENCY INJECTION (20 questions) ───────────────────────────

const SERVICES: Row[] = [

t('angular-ext-t-004', 'Dependency injection — providers, scopes, and tokens', 'Services & DI', 'hard', `
...
t('angular-ext-t-005', 'HTTP client — requests, interceptors, and error handling', 'Services & DI', 'medium', `
## Angular HttpClient
...
t('angular-ext-t-006', 'inject() function — modern Angular DI pattern', 'Services & DI', 'medium', `
## inject() Function
...
mcq('angular-ext-m-003', 'providedIn root', 'Services & DI', 'easy',
  'What does `@Injectable({ providedIn: "root" })` mean?',
  [
    'The service is available only in the root module',
    'A single instance is created and shared across the entire application',
    'The service is provided at the component level',
    'The service is lazy-loaded with the root route',
  ], 1,
  'providedIn: "root" registers the service with the root injector — there is exactly one instance shared by all components, services, and modules in the app.'),

]

// ─── DIRECTIVES & PIPES (20 questions) ────────────────────────────────────────

const DIRECTIVES: Row[] = [

t('angular-ext-t-007', 'Built-in structural directives — *ngIf, *ngFor, *ngSwitch', 'Directives & Pipes', 'easy', `
...
t('angular-ext-t-008', 'Custom directives — attribute and structural', 'Directives & Pipes', 'hard', `
## Custom Directives
...
t('angular-ext-t-009', 'Custom pipes — pure vs impure', 'Directives & Pipes', 'medium', `
## Custom Pipes
...
mcq('angular-ext-m-004', 'Pure vs impure pipe', 'Directives & Pipes', 'medium',
  'When is a pure pipe\'s transform method called?',
  [
    'On every change detection cycle',
    'Only when the input reference or primitive value changes',
    'Only on component initialization',
    'Once per template binding',
  ], 1,
  'Pure pipes are memoized — Angular only calls transform() when the input reference changes (or primitive value changes). This makes them efficient but means they won\'t detect mutations inside arrays/objects.'),
...
t('angular-ext-t-010', 'Reactive forms — FormGroup, FormControl, FormArray', 'Angular Forms', 'medium', `
## Reactive Forms
...
t('angular-ext-t-011', 'Custom validators — sync and async', 'Angular Forms', 'hard', `
## Custom Validators
...
mcq('angular-ext-m-005', 'Reactive vs template-driven forms', 'Angular Forms', 'medium',
  'What is the key advantage of reactive forms over template-driven forms?',
  [
    'They require less code',
    'They work without TypeScript',
    'The model lives in the component class, enabling easier testing and dynamic validation',
    'They automatically handle async validators',
  ], 2,
  'In reactive forms, the form model (FormGroup, FormControl) lives in the component class. This makes it testable, allows dynamic form construction, and provides synchronous access to the form\'s validity and values.'),

]

// ─── ROUTING (20 questions) ───────────────────────────────────────────────────

const ROUTING: Row[] = [

t('angular-ext-t-012', 'Angular routing — lazy loading, guards, resolvers', 'Angular Routing', 'medium', `
## Angular Router
...
t('angular-ext-t-013', 'Route parameters — params, query params, data', 'Angular Routing', 'medium', `
## Reading Route Data
...
mcq('angular-ext-m-006', 'Lazy loading benefit', 'Angular Routing', 'easy',
  'What is the primary benefit of lazy-loading feature modules?',
  [
    'Faster build times',
    'Smaller initial bundle — chunks are loaded on demand',
    'Simpler routing configuration',
    'Enables server-side rendering',
  ], 1,
  'Lazy loading splits the app into chunks. Only the main bundle is loaded initially; feature chunks load when the user navigates to that route. This significantly reduces Time to Interactive for large apps.'),

]

// ─── CHANGE DETECTION & SIGNALS (20 questions) ────────────────────────────────

const CHANGE_DETECTION: Row[] = [

t('angular-ext-t-014', 'Change detection — Default vs OnPush strategies', 'Change Detection & Signals', 'hard', `
...
t('angular-ext-t-015', 'Angular Signals — reactive state without RxJS', 'Change Detection & Signals', 'hard', `
...
mcq('angular-ext-m-007', 'OnPush trigger', 'Change Detection & Signals', 'hard',
  'A component uses OnPush. Its @Input is an array. A parent pushes an item into the array (mutation). Will the child re-render?',
  [
    'Yes — array contents changed',
    'No — the array reference did not change',
    'Yes — OnPush tracks deep changes',
    'Only if the array length changes',
  ], 1,
  'OnPush checks INPUT REFERENCE equality. Mutating an array (push, splice) does not change the reference, so Angular skips the child. Always pass a new array: `[...items, newItem]`.'),

]

// ─── Export ───────────────────────────────────────────────────────────────────

export const allAngularMoreProblems: Row[] = [
  ...COMPONENTS,
  ...SERVICES,
  ...DIRECTIVES,
  ...FORMS,
