...
function t(
  ...
  return {
    ...
    tags: JSON.stringify([topic, 'rxjs']),
    questionType: 'theory', subject: 'rxjs',
...
function mcq(
  ...
  return {
    ...
    tags: JSON.stringify([topic, 'rxjs', 'mcq']),
    questionType: 'mcq', subject: 'rxjs',
...
t('rxjs-theory-001', 'RxJS Core Concepts — Observable, Observer & Subscription', 'RxJS Basics', 'easy', `
## RxJS Core Concepts
...
t('rxjs-theory-002', 'Transformation Operators — map, switchMap, mergeMap, concatMap, exhaustMap', 'Operators', 'hard', `
...
t('rxjs-theory-003', 'Filtering & Combination Operators', 'Operators', 'medium', `
...
t('rxjs-theory-004', 'Error Handling in RxJS', 'Error Handling', 'hard', `
## RxJS Error Handling
...
t('rxjs-theory-005', 'Subjects Deep Dive & Multicasting', 'Subjects', 'hard', `
...
t('rxjs-theory-006', 'Schedulers & Custom Operators', 'Advanced RxJS', 'hard', `
...
t('rxjs-theory-007', 'RxJS Patterns in Real Applications', 'Patterns', 'hard', `
...
mcq('rxjs-mcq-001', 'Cold vs Hot observable', 'RxJS Basics', 'medium',
  'Which of the following is a HOT observable?',
  [
    '`of(1, 2, 3)`',
  ...
  2,
  '`fromEvent` is hot — DOM events happen independently of subscribers. All subscribers share the same stream of events. `of`, `http.get`, and `new Observable` are cold — each subscriber gets its own independent execution.'),

mcq('rxjs-mcq-002', 'switchMap cancellation', 'Operators', 'hard',
  'You use `switchMap` to make an HTTP request on each keypress. What happens when the user presses a key while the previous request is still pending?',
  [
    'Both requests complete and results are merged',
    'The new request waits in a queue until the previous completes',
    'The previous request is cancelled (unsubscribed) and a new one starts',
    'The new keypress is ignored until the previous request completes'
  ],
  2,
  '`switchMap` "switches" to the new inner Observable by unsubscribing from the previous one. For HTTP requests (which are Observables), Angular\'s HttpClient cancels the in-flight request via AbortController when unsubscribed. This prevents race conditions in search inputs.'),

mcq('rxjs-mcq-003', 'BehaviorSubject initial value', 'Subjects', 'easy',
  'What does a new subscriber receive immediately upon subscribing to a `BehaviorSubject("initial")`?',
  [
    'Nothing — it waits for the next emission',
    '"initial"',
    'An error because no value was emitted yet',
    'undefined'
  ],
  1,
  'BehaviorSubject requires an initial value and ALWAYS emits the current value to new subscribers immediately upon subscription. This is its key difference from plain Subject (which only emits future values).'),

mcq('rxjs-mcq-004', 'shareReplay purpose', 'Subjects', 'hard',
  'You have `const data$ = http.get("/api/config").pipe(shareReplay(1))`. Two components subscribe. How many HTTP requests are made?',
  [
    '2 — one per subscriber',
    '1 — the response is cached and replayed to the second subscriber',
    '0 — shareReplay prevents requests',
    'Depends on subscription timing'
  ],
  1,
  '`shareReplay(1)` multicasts the source and caches the last 1 emission. The first subscriber triggers one HTTP request. The second subscriber immediately receives the cached response without a new request. This is the standard pattern for caching configuration/reference data.'),

mcq('rxjs-mcq-005', 'combineLatest vs withLatestFrom', 'Operators', 'hard',
  'What is the key difference between `combineLatest([a$, b$])` and `a$.pipe(withLatestFrom(b$))`?',
  [
    'No difference — they produce the same result',
    'combineLatest emits when EITHER source emits; withLatestFrom only emits when the primary (a$) emits',
    'withLatestFrom waits for both sources to complete',
    'combineLatest requires all sources to emit at the same time'
  ],
  1,
  '`combineLatest` emits whenever ANY source emits (requires all to have emitted at least once). `withLatestFrom` only emits when the PRIMARY observable (a$) emits — b$ is sampled but doesn\'t trigger emissions. Use withLatestFrom when clicks should trigger an action using the current state of another observable.'),

mcq('rxjs-mcq-006', 'takeUntil vs unsubscribe', 'Operators', 'medium',
  'Why is `takeUntil(this.destroy$)` preferred over manually calling `subscription.unsubscribe()` in Angular?',
  [
    'takeUntil is faster than unsubscribe',
    'takeUntil also cancels inner observables from switchMap/mergeMap; single point of cleanup for multiple subscriptions',
    'unsubscribe does not work with HTTP observables',
    'takeUntil is required for OnPush change detection'
  ],
  1,
  '`takeUntil` completes the observable chain cleanly — it also completes any inner Observables created by flattening operators. With multiple subscriptions, one `destroy$.next()` completes all of them. Manual unsubscribe requires tracking every subscription individually and doesn\'t propagate completion through the pipeline.'),

mcq('rxjs-mcq-007', 'tap operator purpose', 'Operators', 'easy',
  'What is the purpose of the `tap` operator?',
  [
    'Transform values in the stream',
    'Perform side effects (logging, debugging) without modifying the stream',
    'Filter values from the stream',
    'Combine multiple observables'
  ],
  1,
  '`tap` (formerly `do`) performs side effects for each emission WITHOUT modifying the values that pass through. It\'s transparent — values flow through unchanged. Common uses: logging, debugging, setting loading state, analytics. Never use tap to transform values — use map instead.'),

mcq('rxjs-mcq-008', 'forkJoin behavior on error', 'Operators', 'hard',
  'You have `forkJoin([requestA$, requestB$, requestC$])`. If requestB$ errors, what happens?',
  [
    'Returns results from A and C, and null for B',
    'Waits for A and C to complete, then emits the error',
    'Immediately errors — A and C are cancelled',
    'Retries requestB$ automatically'
  ],
  2,
  '`forkJoin` errors immediately if ANY source errors, similar to `Promise.all`. The other observables are cancelled (unsubscribed). If you want to handle errors per-source, use `catchError` on each individual observable before passing to forkJoin.'),
]

export const allRxjsProblems: Row[] = [...THEORY, ...MCQS]

export async function seedRxjs(d1: D1Database): Promise<number> {
  const db = drizzle(d1)
  const BATCH = 20
  let inserted = 0
  for (let i = 0; i < allRxjsProblems.length; i += BATCH) {
    const batch = allRxjsProblems.slice(i, i + BATCH)
